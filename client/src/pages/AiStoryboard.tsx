...
const slides = [
  ...
  {
    num: '02',
    img: '/previz-storyboard-director-strategy.webp',
    altEn: 'Director strategy storyboard contact sheet',
    altCn: '导演策略分镜联系单',
    tagEn: 'WONG KAR-WAI STYLE',
    tagCn: '王家卫风格',
    titleEn: 'Original + Director Strategy',
    titleCn: '原链路 + 导演策略',
    type: 'mixed' as const,
    pointsEn: [
  ...
  },
  {
    ...
    tagCn: '多智能体架构',
    titleEn: 'New Pipeline + Director Strategy',
    titleCn: '新链路（多智能体）+ 导演策略',
    type: 'win' as const,
    pointsEn: [
    ...
    <div className="border-t border-[#E5E5E5] py-20 md:py-24">
      <div className="container">
        ...
        <p className="text-[#6B6B6B] text-sm font-body leading-relaxed mb-12 max-w-2xl">
          {t(
            'Three versions of the same scene — generated by the original pipeline, the original pipeline with director strategy analysis (Wong Kar-wai style), and the new multi-agent pipeline with director strategy analysis.',
            '同一场景的三个版本分别由原链路、加入导演策略分析的原链路（王家卫风格），以及加入导演策略分析的多智能体新链路生成。'
          )}
        </p>
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <div className="pt-24">
        ...
        <div className="container pt-16 pb-0">
          <div className="max-w-4xl">
            ...
            <h1 className="font-heading font-800 text-[#0A0A0A] text-4xl md:text-6xl leading-none tracking-tight mb-8">
              {t('AI Storyboard', '剧本到分镜')}<br />
              {t('Generation System', '生成流程')}
            </h1>
            <p className="text-[#6B6B6B] text-base md:text-lg font-body leading-relaxed max-w-2xl mb-10">
              {t(
                'Contributed to the product decomposition and workflow design of a script-to-storyboard system, focusing on directing logic, reusable skills, cross-shot context and project-case evaluation.',
                '参与剧本到分镜能力的产品拆解与流程设计，将剧情理解、资产提取、视觉方案、镜头规划、分镜生成和回修组织为可操作的创作流程。'
              )}
            </p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-0 border border-[#E5E5E5]">
              {[
                { label: 'TYPE', value: t('Creative workflow', '创作流程设计') },
                { label: 'FOCUS', value: t('Workflow · Skill · Context', '工作流 · Skill · 上下文') },
                { label: 'STATUS', value: t('Project-case validation', '项目案例验证') },
                { label: 'NEXT', value: t('Ongoing iteration', '持续迭代') },
              ].map((item, i) => (
        ...
        <CutRoom title={t('Storyboard Assembly', '分镜剪辑台')} asset={STORYBOARD_IMG} unit="MULTI-AGENT" />

        <section className="border-t border-[#E5E5E5] bg-[#101716] py-16 text-white md:py-20">
          <div className="container">
            <div className="grid gap-10 lg:grid-cols-[0.72fr_1.28fr]">
              <div className="max-w-md">
                <p className="font-mono-custom text-[9px] tracking-[0.16em] text-[#d6c07f]">CONTRIBUTION BOUNDARY</p>
                <h2 className="mt-4 font-heading text-3xl font-700 leading-tight md:text-4xl">{t('What the team built, and what I worked on.', '团队系统与我的实际工作分别是什么')}</h2>
                <p className="mt-5 text-sm leading-7 text-white/62">{t('The page separates product context, personal contribution and evidence so that system-scale outcomes are not presented as individual ownership.', '页面将团队系统背景、个人参与部分与可验证证据分开呈现，避免把整体系统规模直接归因于个人。')}</p>
              </div>
              <div className="grid gap-px bg-white/16 md:grid-cols-3">
                {[
                  { label: t('TEAM CONTEXT', '团队系统背景'), title: t('Script-to-storyboard capability', '剧本到分镜能力'), copy: t('A production workflow serving real project cases in WorkRally.', '面向 WorkRally 真实项目使用的剧本到分镜生产能力。') },
                  { label: t('MY CONTRIBUTION', '我的参与部分'), title: t('Workflow, Skill and context', '流程、Skill 与上下文'), copy: t('Product decomposition, directing rules, reusable Skill design, cross-shot context and bad-case analysis.', '参与产品拆解、导演规则、可复用 Skill、跨镜头上下文与 Bad Case 分析。') },
                  { label: t('EVIDENCE', '可验证证据'), title: t('Cases and evaluation', '案例与评估'), copy: t('Compare generations from the same script and trace issues back to prompts, references, state and revision nodes.', '通过同剧本多版本对比，将问题回溯到提示词、参考图、状态与回修节点。') },
                ].map((item) => <article key={item.label} className="bg-[#101716] p-6 md:p-7"><p className="font-mono-custom text-[8px] tracking-[0.15em] text-white/42">{item.label}</p><h3 className="mt-4 font-heading text-xl font-700">{item.title}</h3><p className="mt-3 text-sm leading-6 text-white/62">{item.copy}</p></article>)}
              </div>
            </div>
          </div>
        </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-12">
              <p className="text-[#6B6B6B] text-sm font-body leading-relaxed">
                {t(
                  ...
                  '我参与将剧本到分镜的创作过程拆成剧情与场次理解、人物场景道具提取、导演意图与视觉方案、镜头规划、分镜生成、视频 Prompt 和回修等节点。每个节点都有明确的输入、输出、依赖与人工确认点。'
                )}
              </p>
              <p className="text-[#6B6B6B] text-sm font-body leading-relaxed">
                {t(
                  'Director strategy analysis draws on representative films to identify shot strategy, framing logic and visual design patterns.',
                  '我结合《傀戏》《繁花》等项目案例验证剧情拆解、镜头设计和视觉方案，并根据生成与重抽中的问题，回看提示词、参考图、指令遵循和一致性等环节。'
                )}
              </p>
            </div>
            ...
            <div className="border border-[#E5E5E5] bg-[#FAFAFA] p-6 md:p-10">
              ...
              <svg
                ...
                >
                  ...
                  <text x="470" y="46" textAnchor="middle" fontSize="9" fill="#CC0000" letterSpacing="1">ANALYSIS</text>
                  <text x="470" y="67" textAnchor="middle" fontSize="15" fontWeight="700" fill="#0A0A0A">{t('Creative Cases', '创作案例')}</text>
        ...
        <div className="border-t border-[#E5E5E5] py-20 md:py-28 bg-[#FAFAFA]">
          <div className="container">
            <div className="flex items-center gap-4 mb-16">
              ...
              <div className="editorial-line flex-1" />
              <h2 className="font-heading font-700 text-[#0A0A0A] text-2xl md:text-3xl tracking-tight">
                {t('Directing Method Translation', '导演方法转译')}
              </h2>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
              <div>
                <p className="text-[#6B6B6B] text-sm font-body leading-relaxed mb-6">
                  {t(
                    'Creative cases are used to test whether story decomposition, visual direction and shot planning serve the narrative.',
                    '以具体剧本和分镜案例验证剧情拆解、视觉方案与镜头规划是否真正服务叙事。'
                  )}
                </p>
                ...
                <blockquote className="border-l-2 border-[#0A0A0A] pl-6 mb-6">
                  <p className="text-[#0A0A0A] text-sm font-body leading-relaxed italic">
                    {t(
                      '"A directing method needs to retain its trigger, decision logic and a way to verify the resulting shot."',
                      '“导演方法需要保留触发条件、决策逻辑和结果验证，才能进入产品流程。”'
                    )}
                  </p>
                </blockquote>
                ...
                <p className="text-[#6B6B6B] text-sm font-body leading-relaxed">
                  {t(
                    'The current work is validated through script and storyboard cases, then refined according to whether the output supports the story.',
                    '当前通过剧本与分镜 Case 验证，再根据生成镜头是否服务剧情调整规则颗粒度。'
                  )}
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                {[
                  ...
                  {
                    step: '04',
                    en: 'Strategy Documentation',
                    cn: '策略整理',
                    descEn: 'Document extracted strategies by genre, mood, scene type and director for use during storyboard design.',
                    descCn: '按题材、情绪、场景类型与导演整理提取出的策略，供分镜设计时参考。',
                  },
                ].map((item) => (
          ...
          <div className="container">
            ...
            <p className="text-[#6B6B6B] text-sm font-body leading-relaxed max-w-2xl mb-16">
              {t(
                ...
                '流程把复杂导演任务拆成可编辑的创作节点。创作者可以在视觉设定或镜头规划层局部修改，避免因为某一处不满意而重新生成整条链路。'
              )}
            </p>
              ...
              <div className="flex items-start gap-6">
                ...
                <div className="flex-1 border-l border-[#E5E5E5] pl-6">
                  <p className="text-[#6B6B6B] text-xs font-body leading-relaxed">
                    {t('A reusable production module that reads directing, art, camera and editing outputs; writes one structured shot script; applies a shared prompt schema and cross-batch state; and keeps human review before generation and local revision.', '一个可复用的生产模块：读取导演、美术、摄影与剪辑节点输出，写入统一结构的分镜脚本；约束提示词格式与跨批次状态，并在生成前和局部回修时保留人工确认。')}
                  </p>
                  <dl className="mt-5 grid gap-3 border-t border-[#E5E5E5] pt-4 sm:grid-cols-2">
                    {[
                      [t('TRIGGER', '触发'), t('Creative direction confirmed', '创作方向确认后')],
                      [t('READS', '读取'), t('Role outputs + project state', '节点输出＋项目状态')],
                      [t('WRITES', '写入'), t('Structured shot script', '结构化分镜脚本')],
                      [t('GUARDRAIL', '人工确认'), t('Before generation + local revision', '生成前＋局部回修')],
                    ].map(([label, value]) => <div key={label}><dt className="font-mono-custom text-[8px] tracking-[0.14em] text-[#AAAAAA]">{label}</dt><dd className="mt-1 text-xs text-[#353535]">{value}</dd></div>)}
                  </dl>
                </div>
              </div>
            </div>
        ...
        <div className="border-t border-[#E5E5E5] py-20 md:py-28 bg-[#FAFAFA]">
          <div className="container">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16">
              <p className="text-[#6B6B6B] text-sm font-body leading-relaxed">
                {t(
                  'The evaluation system records issues in generated storyboards and incorporates expert feedback into prompt templates and evaluation rules for the next cycle.',
                  '评估体系记录生成分镜中的问题，并将专家反馈整理进 Prompt 模板和评估规则，用于下一轮优化。'
                )}
              </p>
              <p className="text-[#6B6B6B] text-sm font-body leading-relaxed">
                {t(
            ...
            <div className="border border-[#E5E5E5] bg-white p-6 md:p-10 mb-16">
              ...
              >
                ...
                <text x="350" y="576" textAnchor="middle" fontSize="10" fill="#888">{t('Prompt templates · Evaluation rules · Scoring weights', '更新 Prompt · 评估规则 · 评分权重')}</text>
              ...
              {[
                {
                  ...
                  cn: '归因分析',
                  descEn: 'For each issue missed by the Agent, identify whether the cause is a prompt gap, a missing rule category or insufficient example coverage.',
                  descCn: '针对 Agent 遗漏的每个问题，判断原因是 Prompt 缺口、规则类别缺失，还是案例覆盖不足。',
                },
                {
                  step: '02',
                ...
                },
                {
                  ...
                  cn: '格式化为机器可读',
                  descEn: 'Structure each rule with a natural-language description, detection logic, severity level and annotated negative samples.',
                  descCn: '将每条规则整理为自然语言描述、检测逻辑、严重程度和标注负样本。',
                },
                {
                  step: '04',
                  en: 'Integration & Validation',
                  cn: '集成与验证',
                  descEn: 'New rules are added to the Agent\'s prompt template and evaluation rules. The next cycle verifies whether the Agent detects the previously missed issue type.',
                  descCn: '新规则会加入 Agent 的 Prompt 模板和评估规则。下一个评估周期验证 Agent 是否能识别此前遗漏的问题类型。',
                },
              ].map((item) => (
            ...
            <blockquote className="border-l-2 border-[#0A0A0A] pl-6">
              <p className="text-[#0A0A0A] text-sm font-body leading-relaxed italic">
                {t(
                  '"Record what the agent misses during evaluation, then turn reusable professional judgment into rules."',
                  '“评估时记录 Agent 遗漏的问题，再将可复用的专业判断整理为规则。”'
                )}
              </p>
          ...
          <div className="container">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              <div>
                ...
                <div className="space-y-3">
                  {[
                    { en: 'Storyboard workflow decomposed into clear creative nodes', cn: '已完成剧本到分镜流程的关键节点拆解' },
                    { en: 'Creative cases used for validation', cn: '已使用具体剧本与分镜 Case 进行验证' },
                    { en: 'Directing knowledge translated into reusable rules', cn: '已参与将导演方法转译为可复用的规则表达' },
                    { en: 'Human confirmation points retained in the workflow', cn: '流程中保留了人工确认与局部回修节点' },
                  ].map((item, i) => (
                    <div key={i} className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {[
                    { en: 'Validate the workflow with more narrative cases', cn: '继续用更多叙事 Case 验证流程' },
                    { en: 'Refine the input, output and dependencies of each node', cn: '继续细化各节点的输入、输出和依赖关系' },
                    { en: 'Improve shot revision and continuity handling', cn: '持续优化镜头回修与连续性处理' },
                    { en: 'Keep evaluating whether generated shots serve the story', cn: '持续判断生成镜头是否真正服务叙事' },
                  ].map((item, i) => (
                    <div key={i} className="flex items-start gap-3">
